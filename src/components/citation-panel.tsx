...
export function CitationPanel({ locator, resourceId }: { locator?: Locator; resourceId?: string }) {
  ...
  async function generate(nextStyle: typeof style) {
    setStyle(nextStyle); setBusy(true); setResult(null);
    try {
      ...
      const citation = await response.json() as CitationResult;
      setResult(citation);
      if (!citation.ok) toast.error("Citation formatting failed. Review the metadata and retry.");
    } catch { toast.error("Citation metadata is unavailable. Please retry."); }
  ...
  return <div className="mt-4 rounded-xl border border-border bg-muted/30 p-4">
    ...
    {result?.ok && <div className="mt-3 space-y-2"><p className="whitespace-pre-wrap break-words text-sm">{result.text}</p><button type="button" onClick={() => void navigator.clipboard.writeText(result.text).then(() => toast.success("Citation copied." )).catch(() => toast.error("Could not copy citation."))} className="min-h-10 rounded-lg border border-border px-3 text-sm font-medium hover:bg-muted">Copy citation</button></div>}
